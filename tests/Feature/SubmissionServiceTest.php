...
it('creates a submission with the editable fields and stores the CV under the token folder', function () {
    ...
    $submission = $this->service->saveDraft($this->token, [
...
it('updates the existing submission instead of creating a duplicate', function () {
    $first = $this->service->saveDraft($this->token, [
        'position_id' => $this->position->id,
...
it('replaces the previous CV by deleting the old file before storing the new one', function () {
    ...
    $this->service->saveDraft($this->token, ['position_id' => $this->position->id], $oldCv);
...
it('marks submitted_at on the first complete submission and only last_updated_at thereafter', function () {
    ...
    $first = $this->service->saveDraft($this->token, ['position_id' => $this->position->id], $cv);
...
it('keeps the submission in draft when no CV has been uploaded yet', function () {
    $submission = $this->service->saveDraft($this->token, [
        'position_id' => $this->position->id,
        'current_structure' => 'Brouillon',
    ], null);
...
it('rejects a CV that is not a PDF', function () {
    ...
    expect(fn () => $this->service->saveDraft($this->token, ['position_id' => $this->position->id], $cv))
...
it('adds a diploma stored in the diplomas subfolder of the token', function () {
    $submission = $this->service->saveDraft($this->token, ['position_id' => $this->position->id], null);
